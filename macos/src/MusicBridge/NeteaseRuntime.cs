...
internal static class NeteaseRuntime
{
    ...
    private static string _lastFmError;
    ...
    static NeteaseRuntime()
    {
        ...
        Fm.Changed += () => {
            if (!string.IsNullOrEmpty(Fm.Error) && Fm.Error != _lastFmError) BridgeLog.Warn("私人FM错误：" + Fm.Error);
            _lastFmError = Fm.Error;
        };
    ...
    {
        Context?.Invalidate(); AudioPlayer.Instance?.Stop(); Fm.End(); Context = context; Favorites.Bind(context);
    }
    public static void Shutdown()
    { Context?.Invalidate(); AudioPlayer.Instance?.Stop(); Context = null; Fm.End(); Favorites.Bind(null); }
